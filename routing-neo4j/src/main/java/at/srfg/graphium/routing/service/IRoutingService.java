 ...
 *  @author mwimmer
 */
public interface IRoutingService<T extends IWaySegment> {
	 ...
	 * 
	 * @param options options for algorithm (e.q. cost criteria used to calcualate the best route (e.g. time or length))
	 * @param startNodes the nodes representing segments in the graph the route(s) starts
	 * @param endNodes the nodes representing segments in the graph the route(s) ends
	 * @return the found route or null if no route could be found.
	 *
	 */
	IRoute<T> route(IRoutingOptions options,
			List<Node> startNodes, List<Node> endNodes);
